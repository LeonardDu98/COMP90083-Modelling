...
[
  ...
  cured?              ;; If true, the person has lived through an infection.
                      ;; They can turn to susceptiable after a long time

  ever-infected?      ;; If true, the person was or is infected
  ever-recoveried?    ;; If true, the person was or is recoveried

  susceptible-at-first? ;; Tracks whether the person was initially susceptible
  ...
  immunization-length ;; How long the person has been cured
  immunization-time   ;; Time (in hours) it takes before the person has a chance to susceptible from the cured

  suspected?          ;; If true, the person is infected but not comfirmed
  confirmed?          ;; If true, the person is comfirmed.
]
...
to setup-people
  ...
  [
    ...
    set cured? false
    set infected? false
    set ever-recoveried? false
    set ever-infected? false
    set susceptible? true
    set suspected? false
    set confirmed? false
    set susceptible-at-first? true
    ...
    ;; Set the recovery time for each agent to fall on a
    ;; normal distribution around average recovery time
    set immunization-time random-normal average-immunization-time average-immunization-time / 4

    ;; make sure it lies between 0 and 2x average-recovery-time
    if immunization-time > average-immunization-time * 2 [
      set immunization-time average-immunization-time * 2
    ]
    if immunization-time < 0 [ set immunization-time 0 ]
    ...
    [
      ...
      set susceptible-at-first? false
...
;; suspected people has a chance to take nucleic-acid-test
;; to be confirmed
to nucleic-acid-test
  if suspected?
  [
    if (random-float 2 < 1)[
      set suspected? false
...
to maybe-recover
  ...
  [
    ...
    [ set infected? false
      ...
      set confirmed? false
      set cured? true
      set ever-recoveried? true
      set immunization-length 0
      set nb-recovered (nb-recovered + 1)
    ]
  ]
end

to maybe-susceptible
  set immunization-length immunization-length + 1

  ;; If people have been cured for more than the immunization-time
  ;; then there is a chance for susceptible
  if immunization-length > immunization-time
  [
    if random-float 100 < susceptible-chance[
      set cured? false
      set susceptible? true
    ]
  ]
...
to calculate-r0
  ...
  let s0 count turtles with [susceptible-at-first?]
...
"" ""
PENS
"% infected" 1.0 0 -2674135 true "" "plot (((count turtles with [ ever-infected? ]) / initial-people) * 100)"
"% recovered" 1.0 0 -10899396 true "" "plot ((count turtles with [ ever-recoveried? ] / initial-people) * 100)"
